...
int s = 0;	//보더팔로우1(원본 이진화 사용한것) 성공하면 0 실패하면 1
int t = 0;	//보더팔로우2(차영상 사용한것) 성공하면 0 실패하면 1
//////////////////////////////////////////////////////////////////////////////////////
bool m_IsRed = TRUE;
bool m_IsGreen = TRUE;
bool m_IsBlue = TRUE;
...
void m_BorderFollow(int height, int width, LPBYTE src, LPBYTE dest, CString Color)
{
	...
	for (x = 1; x < height; x++)
	{
		for (y = 1; y < width; y++)
		{
			...
			if (c0 != c1 && c0 == (unsigned char)255 && visited[x*width + y] == 0)  //c0 != c1 (경계이고), c1은 하얀색, 방문하지 않은점 이면
			{
				...
				do
				{
					...
					for (k = 0; k < 8; k++, n = ((n + 1) & 7))  // 01234567	0~7까지 범위
					{									 // 12345670
						short  u = (short)(x + nei[n].x);
						short  v = (short)(y + nei[n].y);
						if (u < 0 || u >= height || v < 0 || v >= width) continue;
						if (src[u * width + v] == c0) break; // 관심점의 주위를 돌다가 같은 밝기의 경계를 만나면 다음으로 추적할 점이 된다. 
					}
					...
					if (border_count >= 10000) break;	//너무 큰크기는 쓰지 않음
